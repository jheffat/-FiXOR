...
def helpscr():
    ...
    sleep(2)
    exit("\nExit...\n")
Password="";n=0;targets=[];op="";banfilels=[];sucessed=[];notsucessed=[] ;lensuc=0  ;decryptdata=bytearray();encryptdata=bytearray();statuspass="";state=False ;N_integrity=0      ;esc=0 ;posbyte=0                       
    ...
    for Filename in targets:
        ...
        if checkpass(Password.encode(),passhashed,Salted.encode(),100,32):
            try:       
                ...
                print(f"| Integrity: ✅ {N_integrity}  ⛔ {lensuc-N_integrity}")
                print('\r[%s%s] ' % ('█' * int(lensuc*65/lentarg), '░'*(65-int(lensuc*65/lentarg))),  end='\n')
                print(f"\n| Target: 📝{path.basename(Filename)}")
